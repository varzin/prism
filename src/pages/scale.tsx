...
export function Scale() {
  ...
  React.useEffect(() => {
    ...
    function handleKeyDown(event: KeyboardEvent) {
      ...
      // arrows and the center panel's Alt+Arrow scale switch below.
      ...
      if (activePanel === 'center' && event.altKey && (event.key === 'ArrowUp' || event.key === 'ArrowDown')) {
        event.preventDefault()
        switchScale(event.key === 'ArrowDown' ? 1 : -1)
        return
      }
  ...
  // Switching scales (left panel arrows, or Alt+Arrow in the center panel)
  // changes scaleId; keep the equivalent thing focused in the new scale - the
  // scale's link on the left, or the same point index on the active curve.
  React.useEffect(() => {
    if (activePanel === 'left') {
      document.getElementById(`scale-link-${scaleId}`)?.focus()
